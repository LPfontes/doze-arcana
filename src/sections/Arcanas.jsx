...
const RUNES_BY_GRAU = {
    grau1: {
        ...
        customRunes: [
            ...
            {
                element: ' Vínculo',
                colorClass: 'from-indigo-500/10 to-indigo-600/5 border-indigo-200 text-indigo-900 bg-indigo-50/15',
                runes: [
            ...
            {
                element: ' Isolamento',
                colorClass: 'from-zinc-500/10 to-zinc-600/5 border-zinc-200 text-zinc-900 bg-zinc-50/15',
                runes: [
            ...
            {
                element: ' Escuridão',
                colorClass: 'from-slate-500/10 to-slate-600/5 border-slate-200 text-slate-900 bg-slate-50/15',
                runes: [
                                ...
                                <div className="mb-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {grau.baseRunes.map((rune, idx) => (
                                            <div key={idx} className="bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm hover:shadow-md transition-all">
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className="font-bold text-indigo-900">{rune.name}</span>
                                                    <span className="bg-indigo-100 text-indigo-800 text-xs px-2 py-0.5 rounded font-mono font-bold">{rune.cost}</span>
                                                </div>
                                                <div className="flex flex-wrap gap-2 mb-2 text-xs">
                                                    {rune.type && <span className="bg-white border border-slate-200 px-1.5 py-0.5 rounded text-slate-600">Tipo: {rune.type}</span>}
                                                    {rune.test && <span className="bg-red-50 border border-red-100 px-1.5 py-0.5 rounded text-red-700">Teste: {rune.test}</span>}
                                                    {rune.time && <span className="bg-amber-50 border border-amber-100 px-1.5 py-0.5 rounded text-amber-700">Tempo: {rune.time}</span>}
                                                </div>
                                                <p className="text-slate-700 mt-2">{rune.desc}</p>
                                                {rune.succ && <p className="text-slate-500 mt-1 italic text-xs">Sucesso: {rune.succ}</p>}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="mb-8">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                        {grau.modifiers.map((mod, idx) => (
                                            <div key={idx} className="bg-slate-50/50 border border-slate-200 p-3 rounded-lg text-sm">
                                                <div className="flex justify-between items-center mb-1">
                                                    <span className="font-bold text-slate-800">{mod.name}</span>
                                                    <span className="text-slate-500 text-xs font-mono">{mod.cost}</span>
                                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-4">
                                        {grau.customRunes.map((elementGroup, idx) => (
                                            <div key={idx} className={`bg-gradient-to-r ${elementGroup.colorClass} border rounded-xl p-4`}>
                                                <h6 className="font-bold text-lg mb-3 flex items-center gap-2">{elementGroup.element}</h6>
                                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                                                    {elementGroup.runes.map((rune, rIdx) => (
                                                        <div key={rIdx} className="bg-white/60 border border-white/40 p-3 rounded-lg shadow-sm text-sm">
                                                            ...
                                                            <p className="text-slate-700 text-xs leading-relaxed">{rune.desc}</p>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
